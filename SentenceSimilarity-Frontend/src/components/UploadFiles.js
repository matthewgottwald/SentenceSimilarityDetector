...
class UploadFiles extends PureComponent {
  ...
  onSubmitFiles = (e) => {
    ...
    this.props.onFilesSubmit(
      filenames,
      this.state.min_samples,
      this.state.epsilon
    );
  };
    ...
    return (
      <div id="myfilepond">
        <form className="ui form" onSubmit={this.onSubmitFiles}>
          <div className="field">
            <label>
              Upload Files (Only upload 1 file if you want to compare against
              AI)
            </label>
            <FilePond
              ...
              labelIdle={
                'Drag and Drop your text files or <span class="filepond--label-action"> Browse </span>'
              }
              ...
              onremovefile={(error, file) => {
                if (this.state.invalidFiles.includes(file)) {
                  const invalidFiles = this.state.invalidFiles.filter(
                    (f) => f !== file
                  );
                  this.setState({ invalidFiles });
                }
          ...
          <div className="field">
            ...
            <p>
              Adjust how similar sentences must be to be included in a cluster
              together
            </p>
            <div id="ClusterInclusivity" className="ui buttons">
              <button
                className={
                  this.state.epsilon === lowEpsilon
                    ? "ui active button"
                    : "ui button"
                }
                onClick={(e) => {
              ...
              <button
                className={
                  this.state.epsilon === standardEpsilon
                    ? "ui active button"
                    : "ui button"
                }
                onClick={(e) => {
              ...
              <button
                className={
                  this.state.epsilon === highEpsilon
                    ? "ui active button"
                    : "ui button"
                }
                onClick={(e) => {
