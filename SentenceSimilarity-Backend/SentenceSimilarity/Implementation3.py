#source: https://stackoverflow.com/questions/53194672/sklearn-dbscan-cosine-vs-precomputed
"""
Sentences are mapped to vectors of TFIDF features. Cosine similarity is computed between each
vector and then DBSCAN clustering is applied.
"""
...
def cluster(X, metric="cosine", eps=0.6, min_samples=3):
    cluster = DBSCAN(
        eps=eps,
        min_samples=3,
        metric=metric,
...
# Using sklearn's built in cosine similarity metric
cluster_assignment = cluster(tfidfs, "cosine").labels_

# Precomputing cosine similarity and then inputting results into DBSCAN algorithm
# (returns same results as above)
distance_matrix = pairwise_distances(tfidfs, tfidfs, metric='cosine')
cluster_assignment2= cluster(csr_matrix(distance_matrix), "precomputed").labels_
